"""Strict MPCC OCP for the standalone rate-control MiL baseline."""
...
from MPCC_baseline_rates.models.quadrotor_mpcc_rate_model import f_system_model_mpcc_rate


def create_mpcc_rate_ocp_description(
    x0, N_horizon, t_horizon, s_max, gamma_pos, gamma_vel, gamma_quat
) -> AcadosOcp:
    """Create the strict MPCC OCP for body-rate commands."""
    ocp = AcadosOcp()
    model, _, _, _ = f_system_model_mpcc_rate()
    ocp.model = model

    proj_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    ocp.code_export_directory = os.path.join(proj_dir, "c_generated_code_mpcc_rate")
