"""Augmented MPCC quadrotor model with rate-control inputs for MiL."""
...
from MPCC_baseline_rates.config.experiment_config import G, MASS, TAU_RC


def f_system_model_mpcc_rate():
    """Build the 14-state MPCC model with body-rate commands."""
    model_name = "Drone_ode_mpcc_rate_mil"
    ...
    dp = vertcat(v1, v2, v3)
    dv = -G * e3 + (rot @ vertcat(MX(0), MX(0), Tt)) / MASS
    ...
    f_x = Function("f0_rate_mpcc", [x], [substitute(f_expl, u, u_zero)])
    g_x = Function("g_rate_mpcc", [x], [jacobian(f_expl, u)])
    f_system = Function("system_rate_mpcc", [x, u], [f_expl])
