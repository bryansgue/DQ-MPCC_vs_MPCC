#!/usr/bin/env python3
"""
run_experiment2_sweep.py — Experiment 2: Velocity Sweep (Monte Carlo).

Runs N_RUNS simulations per speed per controller, computing RMSE_pos
and RMSE_ori for each run.  Saves everything to
  experiment2_results/velocity_sweep_data.mat

NO Experiment-1 files are modified.

Usage:
    python run_experiment2_sweep.py
"""
...
def _run_mpcc_baseline(p0, q0, vtheta_max, solver, f_sys,
    ...
    for k in range(N_sim):
        ...
        e_pos = np.linalg.norm(sd_k - x[0:3, k])
...
def _run_dq_mpcc(p0, q0, vtheta_max, solver, f_sys,
    ...
    for k in range(N_sim):
        ...
        e_pos = np.linalg.norm(sd_k - pos_k)
        pos_errs.append(e_pos)
