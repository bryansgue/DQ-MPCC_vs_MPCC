...
def _run_mpcc_baseline(p0, q0, vtheta_max, solver, f_sys,
    ...
    return dict(
        ...
        xyz         = x[0:3, :k_final+1].copy(),   # (3, K) trajectory
    )
...
def _run_dq_mpcc(p0, q0, vtheta_max, solver, f_sys,
    ...
    return dict(
        ...
        solve_max   = np.max(solver_times),
        solve_std   = np.std(solver_times),
        xyz         = np.array(pos_log).T,   # (3, K) trajectory
    )
...
def main():
    ...
    ics = []
    init_dists = []   # distance from perturbed p0 to nearest path point
    ...
    for _ in range(N_RUNS):
        ...
        # Distance to path start (index 0 on arc-length)
        sd_start = position_by_arc(0.0)
        init_dists.append(np.linalg.norm(p0_pert - sd_start))
    ...
    def _set_vtheta(solver, ubu_template, vtheta_max, N_pred):
        p_val = np.array([vtheta_max])
        ubu = ubu_template.copy()
        ubu[4] = vtheta_max
    ...
    _set_vtheta(mpcc_solver, mpcc_ubu_template, vtheta_max, N_prediction)
    _set_vtheta(dq_solver, dq_ubu_template, vtheta_max, N_prediction)
