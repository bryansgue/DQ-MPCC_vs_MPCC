...
Usage:
    python results/experiment2/scripts/run_experiment2.py
"""
...
from config.experiment_config import (
    ...
    TRAJECTORY_T_FINAL, S_MAX_MANUAL,
)
...
def _run_mpcc_baseline(p0, q0, vtheta_max, solver, f_sys,
    ...
    for k in range(N_sim):
        ...
        # used to build the controller attitude target.
...
def main():
    ...
    if S_MAX is None:
        ...
        s_max = min(float(S_MAX), float(s_max_full))

    # ══════════════════════════════════════════════════════════════════════
    #  BUILD SOLVERS ONCE — use largest v_theta_max for constraints
    # ══════════════════════════════════════════════════════════════════════
    vtheta_max_build = max(VELOCITIES)
    delta_s_terminal = 1.10 * vtheta_max_build * T_PREDICTION
    ...
    mat_dict = {
        ...
        's_max_manual': np.nan if S_MAX_MANUAL is None else float(S_MAX_MANUAL),
        ...
        'seed': int(SEED),
        'controller_dq_name': 'DQ-MPCC',
        'controller_mpcc_name': 'MPCC',
    }
    ...
    mat_dict.update(flatten_weight_summary("mpcc", get_active_weight_summary("mpcc")))
    for ctrl in ['dq', 'base']:
        for v in VELOCITIES:
            ...
            success_count = len(rp)
            failure_count = int(failures[ctrl][v])
            total_count = success_count + failure_count
            success_rate = (success_count / total_count) if total_count > 0 else np.nan
            ...
            mat_dict[f'{ctrl}_v{vstr}_success_count'] = success_count
            mat_dict[f'{ctrl}_v{vstr}_failures']      = failure_count
            mat_dict[f'{ctrl}_v{vstr}_success_rate']  = success_rate
            mat_dict[f'{ctrl}_v{vstr}_failure_rate']  = failure_rate

            if ctrl == 'base':
                for stem, value in (
                    ('rmse_pos', mat_dict[f'{ctrl}_v{vstr}_rmse_pos']),
                    ('rmse_ori', mat_dict[f'{ctrl}_v{vstr}_rmse_ori']),
                    ('rmse_ec', mat_dict[f'{ctrl}_v{vstr}_rmse_ec']),
                    ('rmse_el', mat_dict[f'{ctrl}_v{vstr}_rmse_el']),
                    ('t_lap', mat_dict[f'{ctrl}_v{vstr}_t_lap']),
                    ('mean_vtheta', mat_dict[f'{ctrl}_v{vstr}_mean_vtheta']),
                ):
                    mat_dict[f'mpcc_v{vstr}_{stem}'] = value
                mat_dict[f'mpcc_v{vstr}_success_count'] = success_count
                mat_dict[f'mpcc_v{vstr}_failures'] = failure_count
                mat_dict[f'mpcc_v{vstr}_success_rate'] = success_rate
                mat_dict[f'mpcc_v{vstr}_failure_rate'] = failure_rate
