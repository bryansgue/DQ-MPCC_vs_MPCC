#!/usr/bin/env python3
"""
compare_results.py – Publication-quality comparative plots for DQ-MPCC vs MPCC.
...
derived error/speed signals, and generates three figures suitable for
IEEEtran single-column publication.

If the .mat files are not found, synthetic mock data is generated so that
the script runs standalone for layout prototyping.

All data (real or synthetic) is also saved to a single
'comparison_data.mat' file for MATLAB post-processing.

Usage:
    python compare_results.py            # uses real .mat if available
    python compare_results.py --mock     # force synthetic data
"""
...
_OUTPUT_DIR   = os.path.join(_PROJECT_ROOT, "comparison_figures")
...
def load_real_data():
    ...
    """
    mat_dq   = os.path.join(_PROJECT_ROOT, "Results_DQ_MPCC_baseline_1.mat")
    mat_base = os.path.join(_PROJECT_ROOT, "Results_MPCC_baseline_1.mat")

    # Also check inside subfolders
    if not os.path.isfile(mat_dq):
        mat_dq = os.path.join(_PROJECT_ROOT, "DQ-MPCC_baseline",
                              "Results_DQ_MPCC_baseline_1.mat")
    if not os.path.isfile(mat_base):
        mat_base = os.path.join(_PROJECT_ROOT, "MPCC_baseline",
                                "Results_MPCC_baseline_1.mat")
