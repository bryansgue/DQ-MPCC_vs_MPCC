...
_OUT_DIR = os.path.join(_ROOT, 'experiment2_results')
os.makedirs(_OUT_DIR, exist_ok=True)

try:
    from experiment2_config import VELOCITIES
except ImportError:
    VELOCITIES = [8, 12, 15]
...
def plot_pareto(results, failures, save=True, show=True):
    ...
    for ctrl, label, color, marker, ls in controllers:
        ...
        for xi, yi, vv in zip(x_med, y_med, v_sorted):
            nf  = failures[ctrl][vv]
    ...
    ax.set_ylabel(r'Position RMSE $\|\mathbf{e}_p\|$ [m]  (median over $N$ runs)')
    ...
    subtitle = (r'Error bars = IQR (P25--P75) across Monte Carlo runs  '
                ...
                if _USE_TEX
                     ...
                     '  •  same v_θ_max per pair of points')
    ...
    ax.annotate('', xy=(0.98, 0.06), xytext=(0.88, 0.06),
                xycoords='axes fraction',
                arrowprops=dict(arrowstyle='->', color='gray', lw=1.2))
    ax.text(0.985, 0.06, 'faster', transform=ax.transAxes,
    ...
    if save:
        for ext in ('pdf', 'png'):
            out = os.path.join(_OUT_DIR, f'fig_pareto_rmse_vs_tlap.{ext}')
