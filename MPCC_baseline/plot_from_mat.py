#!/usr/bin/env python3
"""
plot_from_mat.py  —  MPCC: Regenerate all figures from a saved .mat file.

Ejecutar desde dentro de MPCC_baseline/:
    python3 plot_from_mat.py
    python3 plot_from_mat.py Results_MPCC_baseline_1.mat
"""
...
# ── Paths ─────────────────────────────────────────────────────────────────────
_HERE  = os.path.dirname(os.path.abspath(__file__))           # MPCC_baseline/
...
print(f"[MPCC plot]  mat = {mat_path}")

# ── Cargar .mat ───────────────────────────────────────────────────────────────
d = sio.loadmat(mat_path)

# MPCC states: (14, N+1) — primeras 13 filas son el estado estándar [pos, vel, quat, omega]
x_std         = d['states'][:13, :]                    # (13, N+1)
